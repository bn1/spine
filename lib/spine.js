(function() {
  ...
  Model = (function() {
    ...
    Model.find = function(id) {
      ...
      if (!record && ("" + id).match(new RegExp("^" + this.prefix + "\\d+"))) {
    ...
    Model.destroyAll = function(options) {
      ...
      for (key in _ref) {
        value = _ref[key];
        _results.push(this.records[key].destroy(opt));
      }
      return _results;
    };
    ...
    Model.resetIdCounter = function() {
      ...
      lastID = (lastID != null ? typeof lastID.replace === "function" ? lastID.replace(new RegExp("^" + this.prefix), '') : void 0 : void 0) || lastID;
    ...
    function Model(atts) {
      ...
      this.cid || (this.cid = this.id || this.constructor.uid());
    }
    ...
    Model.prototype.save = function(options) {
      ...
      if (options.validate !== false) {
        error = this.validate();
        if (error) {
          if (options.trigger !== false) this.trigger('error', error);
          return false;
        }
      }
      if (options.trigger !== false) this.trigger('beforeSave', options);
      record = this.isNew() ? this.create(options) : this.update(options);
      if (options.trigger !== false) this.trigger('save', options);
      return record;
    };
    ...
    Model.prototype.destroy = function(options) {
      if (options == null) options = {};
      if (options.trigger !== false) this.trigger('beforeDestroy', options);
      delete this.constructor.records[this.id];
      delete this.constructor.crecords[this.cid];
      this.destroyed = true;
      if (options.trigger !== false) this.trigger('destroy', options);
      if (options.trigger !== false) this.trigger('change', 'destroy', options);
      this.unbind();
      return this;
    };
    ...
    Model.prototype.update = function(options) {
      var clone, records;
      if (options.trigger !== false) this.trigger('beforeUpdate', options);
      records = this.constructor.records;
      records[this.id].load(this.attributes());
      clone = records[this.id].clone();
      if (options.trigger !== false) clone.trigger('update', options);
      if (options.trigger !== false) clone.trigger('change', 'update', options);
      return clone;
    };

    Model.prototype.create = function(options) {
      var clone, record;
      if (options.trigger !== false) this.trigger('beforeCreate', options);
      if (!this.id) this.id = this.cid;
      record = this.dup(false);
      this.constructor.records[this.id] = record;
      this.constructor.crecords[this.cid] = record;
      clone = record.clone();
      if (options.trigger !== false) clone.trigger('create', options);
      if (options.trigger !== false) clone.trigger('change', 'create', options);
      return clone;
    };
