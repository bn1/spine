(function() {
  ...
  Model = (function() {
    ...
    Model.prototype.save = function(options) {
      var error, record, trigger;
      if (options == null) options = {};
      trigger = !!options.trigger || true;
      if (options.validate !== false) {
        error = this.validate();
        if (error) {
          if (trigger) this.trigger('error', error);
          return false;
        }
      }
      if (trigger) this.trigger('beforeSave', options);
      record = this.isNew() ? this.create(options) : this.update(options);
      if (trigger) this.trigger('save', options);
      return record;
    };
    ...
    Model.prototype.destroy = function(options) {
      ...
      if (options == null) options = {};
      trigger = !!options.trigger || true;
      if (trigger) this.trigger('beforeDestroy', options);
      delete this.constructor.records[this.id];
      delete this.constructor.crecords[this.cid];
      this.destroyed = true;
      if (trigger) this.trigger('destroy', options);
      if (trigger) this.trigger('change', 'destroy', options);
      this.unbind();
      return this;
    };
    ...
    Model.prototype.update = function(options) {
      var clone, records, trigger;
      trigger = !!options.trigger || true;
      if (trigger) this.trigger('beforeUpdate', options);
      records = this.constructor.records;
      records[this.id].load(this.attributes());
      clone = records[this.id].clone();
      if (trigger) clone.trigger('update', options);
      if (trigger) clone.trigger('change', 'update', options);
      return clone;
    };

    Model.prototype.create = function(options) {
      var clone, record, trigger;
      trigger = !!options.trigger || true;
      if (trigger) this.trigger('beforeCreate', options);
      if (!this.id) this.id = this.cid;
      record = this.dup(false);
      this.constructor.records[this.id] = record;
      this.constructor.crecords[this.cid] = record;
      clone = record.clone();
      if (trigger) clone.trigger('create', options);
      if (trigger) clone.trigger('change', 'create', options);
      return clone;
    };
